...
if model_choice == "SD14":
    sd_pipeline = StableDiffusionPipeline.from_pretrained(pretrained_model_name_or_path=SD14_VERSION).to("cuda:0")
    attn_res = (16, 16)
    max_iter_to_alter = 40
elif model_choice == "SD15":
    sd_pipeline = StableDiffusionPipeline.from_pretrained(pretrained_model_name_or_path=SD15_VERSION).to("cuda:0")
    attn_res = (16, 16)
    max_iter_to_alter = 40
elif model_choice == "SD21":
    sd_pipeline = StableDiffusionPipeline.from_pretrained(pretrained_model_name_or_path=SD21_VERSION).to("cuda:0")
    attn_res = (24, 24)
    max_iter_to_alter = 40
else:
    raise Exception("model_choice should be among SD14 , SD15, SD21.")

pipeline = SelfCrossConformPipeline(
    vae=sd_pipeline.vae,
...
iterative_refinement_steps = [1, 10, 20] # Iterative refinement steps 15, 25
...
for PROMPT in prompts:
    ...
    for SEED in seeds:
        ...
        images = pipeline(prompt=PROMPT, model_choice=model_choice, token_groups=token_groups,guidance_scale=7.5,generator=generator,
            ...
            do_smoothing=False,smoothing_sigma=smoothing_sigma,smoothing_kernel_size=smoothing_kernel_size,
            temperature=temperature,refinement_steps=refinement_steps,softmax_normalize=False,softmax_normalize_attention_maps=False,
            add_previous_attention_maps=True,previous_attention_map_anchor_step=None,loss_fn=loss_fn,result_root=result_root,seed=SEED)
