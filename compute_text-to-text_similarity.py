...
@dataclass
class EvalConfig:
    output_path: Path = Path("../test/animals/")
...
def run(config: EvalConfig):
    ...
    for prompt in tqdm(prompts):
        ...
        with torch.no_grad():
            ...
            cur_prompt_results[prompt] = {
                f'full_text_blip_score': prompt_full_text(results_per_prompt[prompt]),
            }
    ...
    with open(config.metrics_save_path / "blip_prompt_metrics.json", 'w') as f:
...
def prompt_full_text(d):
    """ results for the full text similarity for each prompt. """
    full_text_res = d['text_similarities']
