...
@dataclass
class EvalConfig:
    output_path: Path = Path("../animals_outputs/attention-refocusing")
...
def run(config: EvalConfig):
    ...
    for prompt in tqdm(prompts):
        ...
        with torch.no_grad():
            ...
            if ' and ' in prompt:
                ...
                continue
            #prompt=prompt_parts[1]+" and "+prompt_parts[0]
            print(prompt)
            ...
            #prompt = prompt_parts[0] + " and " + prompt_parts[1]
            ...
            cur_prompt_results[prompt] = {
                f'full_text_clip_score': prompt_full_text(results_per_prompt[prompt]),
                f'min_first_second_clip_score': prompt_min_half(results_per_prompt[prompt]),
            }
    ...
    with open(config.metrics_save_path / "clip_prompt_metrics.json", 'w') as f:
        json.dump(cur_prompt_results, f, sort_keys=True, indent=4)
...
def prompt_min_half(d):
    """ Aggregate results for the minimum similarity score for each prompt. """
    min_per_half_res = [min(a, b) for a, b in zip(d["first_half"], d["second_half"])]
    min_per_half_res = np.array(min_per_half_res).flatten()
    return np.average(min_per_half_res)


def prompt_full_text(d):
    """ Aggregate results for the full text similarity for each prompt. """
    full_text_res = d['full_text']
    full_text_res = np.array(full_text_res).flatten()
    return np.average(full_text_res)
