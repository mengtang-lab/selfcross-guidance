...
class SelfCrossConformPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    r"""
    ...
    def _compute_contrastive_loss(
        ...
        cross_attn_maps = cross_attention_maps[:, :, 1:-1]
        cross_attn_maps = cross_attn_maps * 100
        cross_attn_maps = torch.nn.functional.softmax(cross_attn_maps, dim=-1)
        ...
        for ind, c in indices_to_clases.items():
            ...
            cur_cross_attn_map=cross_attn_maps[:, :, ind-1]
            cross_attn_maps_list.append(cur_cross_attn_map)
            # -----------------------------------
            # clean cross_attention_map_cur_mask
            # -----------------------------------
            topk_coord_list, topk_value_list = fn_get_topk(cur_cross_attn_map, K=4)
            cur_token_mask = fn_get_otsu_mask(cur_cross_attn_map)
            mean_tensor = torch.mean(torch.stack(topk_value_list))
            cross_attn_top_k_values.append(mean_tensor)
            # ---------------------------------
            # prepare aggregated self attn maps
            # ---------------------------------
            self_attn_map_cur_token = torch.zeros_like(embedding)
            for j in range(attention_res):
                for k in range(attention_res):
                    if cur_token_mask[j, k] == 0: continue
                    cur_cross_attn_value = cur_cross_attn_map[j, k]
                    cur_position_self_attn_map = self_attention_maps[j, k].view(attention_res,
                                                                                attention_res).contiguous()
                    self_attn_map_cur_position = cur_position_self_attn_map
                    self_attn_map_cur_token = self_attn_map_cur_token + cur_cross_attn_value * self_attn_map_cur_position
                    #total_weight_cross_attn = total_weight_cross_attn + cross_attn_value_cur_token
            self_attn_maps_list.append(self_attn_map_cur_token/torch.sum(self_attn_map_cur_token)) # / total_weight_cross_attn
            # ------------------------
            # prepare embeddings
            # ------------------------
        ...
        for i in range(len(self_classes)):
            for j in range(len(self_classes)):
                if self_classes[i]==self_classes[j]: continue
                    ...
                    self_attn_maps_list[i] = self_attn_maps_list[i] / (torch.sum(self_attn_maps_list[i])+0.0001)
                    if cross_attn_maps_list[j].shape[0]>16:
                        cross_attn_maps_list[j]=cross_attn_maps_list[j]/(torch.sum(cross_attn_maps_list[j])+0.0001)
                    self_cross_attn_loss = self_cross_attn_loss + torch.min(self_attn_maps_list[i],
                                                                            cross_attn_maps_list[j]).sum()
                    number_self_cross_loss_pair = number_self_cross_loss_pair + 1
        self_cross_attn_loss = self_cross_attn_loss / number_self_cross_loss_pair
        ...
        conloss = loss_fn(embeddings, classes)

        if model_choice=='SD21': # for sd2 the attend and excite loss doesn't work
            if t > 881:
                loss = conloss
            else:
                loss = cross_attn_loss + self_cross_attn_loss
        else:
                ...
                with torch.enable_grad():
                    ...
                    for j, (latent, token_group, text_embedding) in enumerate(
                        ...
                        if result_root is not None:
                            ...
                            # if t==961:self_attention_map_list_list=list
