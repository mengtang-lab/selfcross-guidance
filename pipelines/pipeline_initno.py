...
from diffusers.loaders import StableDiffusionLoraLoaderMixin, TextualInversionLoaderMixin, FromSingleFileMixin, IPAdapterMixin
...
from utils.attn_utils import fn_show_attention, fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask, fn_show_attention_plus
...
class StableDiffusionInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, StableDiffusionLoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
                ...
                with torch.enable_grad():
                    ...
                    for latent, index, text_embedding in zip(latents, indices, text_embeddings):
                        ...
                        if result_root is not None:
                            ...
                            cross_attention_map_numpy, self_attention_map_numpy = fn_show_attention_plus(
            ...
            if result_root is not None:
                ...
                half_row = len(cross_attention_map_numpy_list[0])
                rows = len(cross_attention_map_numpy_list[0]) * 2
                columns = len(cross_attention_map_numpy_list)
                for j in range(len(cross_attention_map_numpy_list)):
                    plt.title("t_{}".format(j))
                    for k in range(len(cross_attention_map_numpy_list[0])):
                        fig.add_subplot(rows, columns, k * columns + j + 1)
                        plt.imshow(cross_attention_map_numpy_list[j][k], cmap='viridis')
                        plt.axis('off')
                    for k in range(len(cross_attention_map_numpy_list[0])):
                        fig.add_subplot(rows, columns, half_row * columns + k * columns + j + 1)
