...
from diffusers.loaders import TextualInversionLoaderMixin, FromSingleFileMixin, IPAdapterMixin, StableDiffusionLoraLoaderMixin
...
from utils.attn_utils import fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask, fn_show_attention_plus
...
class SelfCrossInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, StableDiffusionLoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
        ...
        for i in range(number_token):
            for j in range(number_token):
                if i == j:
                    ...
                    if cross_attention_map.shape[0]>16: cross_attention_map = cross_attention_map/(torch.sum(cross_attention_map)+0.0001)
    ...
    def fn_compute_loss(
        ...
        for i in indices:
            ...
            topk_value_list_list.append(topk_value_list)
            # -----------------------------------
            # clean cross_attention_map_cur_token
            # -----------------------------------
            clean_cross_attention_map_cur_token = cross_attention_map_cur_token
        ...
        for i in range(number_token):
            for j in range(number_token):
                if i == j:
                    ...
                    self_attention_map = self_attention_map_list[j] / torch.sum(self_attention_map_list[j])
                    cross_attention_map = cross_attention_map_list[i]#.detach().clone()
                    if cross_attention_map.shape[0]>16: cross_attention_map = cross_attention_map/torch.sum(cross_attention_map)
                    self_cross_attn_loss_ij = torch.min(self_attention_map, cross_attention_map).sum()
                    number_self_cross_loss_pair = number_self_cross_loss_pair + 1
                    self_cross_attn_loss = self_cross_attn_loss + self_cross_attn_loss_ij
    ...
    def _perform_iterative_refinement_step(
        ...
        while cross_attn_loss > target_loss or self_cross_attn_loss > target_self_loss:
            ...
            joint_loss, cross_attn_loss, self_cross_attn_loss = self.fn_augmented_compute_losss(indices=indices, K=K, attention_res=attention_res)
            if joint_loss != 0: latents = self._update_latent(latents, joint_loss, step_size)
            logging.info(f"\t Try {iteration}. cross loss: {cross_attn_loss:0.4f}. self loss: {self_cross_attn_loss:0.4f}")
    ...
    def fn_initno(
        ...
        for iteration in tqdm(range(max_step)):
            ...
            for i, t in enumerate(timesteps):
                ...
                joint_loss, cross_attn_loss, self_cross_attn_loss = self.fn_compute_loss(indices=indices, K=K, attention_res=attention_res)
                ...
                if denoising_step_for_loss > 1:
                    ...
                    optimized_latents = self.scheduler.step(noise_pred, t, optimized_latents).prev_sample  # , **extra_step_kwargs
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    for latent, index, text_embedding in zip(latents, indices, text_embeddings):
                        ...
                        joint_loss, cross_attn_loss, self_cross_attn_loss = self.fn_augmented_compute_losss(indices=index, K=K, attention_res=attn_res[0])
                        ...
                        if i < max_iter_to_alter and (i == 10 or i == 20 ) and (cross_attn_loss > 0.2 or self_cross_attn_loss > 0.3) and not run_sd:
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred, t, latents).prev_sample  #, **extra_step_kwargs
            ...
            if result_root is not None:
                ...
                for j in range(len(cross_attention_map_numpy_list)):
                    ...
                    plt.axis('off')
                plt.savefig(f"./{result_root}/{prompt}/{seed}_attn.jpg", bbox_inches='tight', pad_inches=0.2)
                plt.close()

                # plt.imshow(cross_attention_map_numpy_list[25][0], cmap='viridis')
                # plt.axis('off')
                # plt.savefig(f"./{result_root}/{prompt}/{seed}_cross_attn0.jpg", bbox_inches='tight', pad_inches=0)
                # plt.close()
                #
                # plt.imshow(cross_attention_map_numpy_list[25][1], cmap='viridis')
                # plt.axis('off')
                # plt.savefig(f"./{result_root}/{prompt}/{seed}_cross_attn1.jpg", bbox_inches='tight', pad_inches=0)
                # plt.close()
                #
                # plt.imshow(self_attention_map_numpy_list[25][0], cmap='viridis')
                # plt.axis('off')
                # plt.savefig(f"./{result_root}/{prompt}/{seed}_self_attn0.jpg", bbox_inches='tight', pad_inches=0)
                # plt.close()
                #
                # plt.imshow(self_attention_map_numpy_list[25][1], cmap='viridis')
                # plt.axis('off')
                # plt.savefig(f"./{result_root}/{prompt}/{seed}_self_attn1.jpg", bbox_inches='tight', pad_inches=0)
                # plt.close()
